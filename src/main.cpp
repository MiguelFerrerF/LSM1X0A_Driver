  ...
  if (controller->begin(verboseEventCallback, nullptr)) {
    ...
    for (int i = 0; i < 5; i++) {
      if (controller->wakeUp()) {
        ...
        ok = controller->sigfox.setRadioPower(14); // 14 dBm
        Serial.printf("Set Radio Power (14dBm): %s\n", ok ? "OK" : "ERROR");

        ok = controller->sigfox.setPublicKeyMode(true); // Enable for testing
        Serial.printf("Set Public Key Mode (TRUE): %s\n", ok ? "OK" : "ERROR");
        ...
        ok = controller->sigfox.sendFrame("48454C4C4F", false, 1); // "HELLO" in HEX
        Serial.printf("-> Send FRAME: %s\n", ok ? "EXITOSO" : "FALLO o TIMEOUT");

        Serial.println("[TEST] Ejecutando Envio Hexadecimal (Payload)...");
        uint8_t payloadData[] = {0xDE, 0xAD, 0xBE, 0xEF};
        ok                    = controller->sigfox.sendHexFrame(payloadData, sizeof(payloadData), false, 1);
        Serial.printf("-> Send HEX: %s\n", ok ? "EXITOSO" : "FALLO o TIMEOUT");
