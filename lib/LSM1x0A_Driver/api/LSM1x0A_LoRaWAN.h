...
public:
  ...
  /**
   * @brief Configures the module's baudrate (default is usually 9600).
  ...
  /**
   * @brief Gets the local time from the module (e.g. "2024-01-01 12:00:00").
   * @param timeinfo Pointer to the standard tm structure where the result will be stored
   * @return true if obtained and parsed correctly.
   */
  bool getLocalTime(struct tm* timeinfo);

  /**
   * @brief Gets the current baudrate configured in the module.
   * @return The baudrate (e.g. 9600) or -1 if it fails.
   */
  int getBaudrate();
...
private:
  ...
  // Cached configuration values to enable state restoration after resets
